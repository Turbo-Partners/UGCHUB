...
export interface IStorage {
  ...
  updateApplicationCreatorWorkflowStatus(id: number, creatorWorkflowStatus: string): Promise<Application>;
  updateApplicationSeeding(id: number, seedingStatus: 'not_required' | 'pending' | 'sent' | 'received', trackingCode?: string, notes?: string): Promise<Application>;
  deleteApplication(id: number): Promise<void>;
  
  // Campaign Briefing Operations
  updateCampaignBriefing(id: number, briefingText: string | null, briefingMaterials: string[] | null): Promise<Campaign>;
  ...
  getCreatorFavoriteCompanies(creatorId: number): Promise<(FavoriteCompany & { company: Company })[]>;
  ...
  getCompanyRecentPartnerships(companyId: number, limit?: number): Promise<{
    id: number;
  ...
  getDiscoverableBrands(options: { category?: string; featured?: boolean; limit?: number; offset?: number }): Promise<{
    id: number;
    name: string;
    tradeName: string | null;
    slug: string | null;
  ...
  getCreatorPendingInvites(creatorId: number): Promise<(CampaignInvite & { campaign: Campaign; company: User })[]>;
  getCreatorAllInvites(creatorId: number): Promise<(CampaignInvite & { campaign: Campaign; company: User })[]>;
  ...
  updateCampaignTemplate(id: number, template: Partial<CampaignTemplate>): Promise<CampaignTemplate>;
  ...
  getUsersWithFilters(filters?: { role?: string; search?: string; isBanned?: boolean; sortBy?: 'name' | 'email' | 'role' | 'createdAt' | 'isBanned'; sortOrder?: 'asc' | 'desc' }): Promise<User[]>;
  updateUserBanStatus(id: number, isBanned: boolean): Promise<User>;
  getProblemReportsWithFilters(filters?: { status?: string }): Promise<(ProblemReport & { user: User })[]>;
  updateProblemReport(id: number, updates: { status?: 'open' | 'in_progress' | 'resolved'; adminNotes?: string }): Promise<ProblemReport>;
  ...
  getUserGrowthStats(days?: number): Promise<Array<{ date: string; creators: number; companies: number }>>;
  ...
  updateCompanyMemberRole(companyId: number, userId: number, role: 'owner' | 'admin' | 'member'): Promise<CompanyMember>;
  ...
  getCompanyUserInviteByToken(token: string): Promise<(CompanyUserInvite & { company: Company }) | undefined>;
  ...
  getCreatorPosts(userId: number, platform?: 'instagram' | 'tiktok', limit?: number): Promise<CreatorPost[]>;
  deleteOldCreatorPosts(userId: number, platform: 'instagram' | 'tiktok', keepCount: number): Promise<void>;
  ...
  updateCreatorHashtagStats(userId: number, platform: 'instagram' | 'tiktok', hashtags: { hashtag: string; avgEngagement: string }[]): Promise<void>;
  ...
  updatePostAiInsight(postId: number, updates: Partial<InsertPostAiInsight>): Promise<PostAiInsight>;
  ...
  getCompanyPublicStats(companyId: number): Promise<{
    ...
    topCreators: { id: number; name: string; avatar: string | null; collaborations: number; avgRating: number }[];
    financialMetrics: {
      totalRevenue: number;
  ...
  recordGamificationEvent(companyId: number, creatorId: number, eventType: string, eventRefId: string, refType: string, refId: number, metadata?: any, campaignId?: number): Promise<PointsLedgerEntry | null>;
  ...
  getCampaignLeaderboardV1(campaignId: number): Promise<(CampaignCreatorStats & { creator: User })[]>;
  getCampaignCreatorStats(campaignId: number, creatorId: number): Promise<CampaignCreatorStats | undefined>;
  upsertCampaignCreatorStats(stats: InsertCampaignCreatorStats): Promise<CampaignCreatorStats>;
  updateCampaignCreatorPoints(campaignId: number, creatorId: number, pointsDelta: number): Promise<CampaignCreatorStats>;
  ...
  getBrandCoupons(brandId: number): Promise<(CampaignCoupon & { creatorName?: string; campaignTitle?: string })[]>;
  ...
  updateCommissionStatus(id: number, status: 'pending' | 'approved' | 'paid'): Promise<CreatorCommission>;
  ...
  addToCompanyWallet(companyId: number, amount: number, description: string): Promise<WalletTransaction>;
  ...
  getCompanyTransactions(companyWalletId: number, filters?: { type?: string; status?: string; userId?: number; limit?: number; offset?: number }): Promise<WalletTransaction[]>;
  getCreatorTransactions(creatorBalanceId: number, limit?: number, offset?: number): Promise<WalletTransaction[]>;
  ...
  // Payment to Creator
  payCreator(companyId: number, creatorUserId: number, amount: number, type: string, description: string, campaignId?: number): Promise<{ companyTransaction: WalletTransaction; creatorTransaction: WalletTransaction }>;
  ...
  getCreatorPendingCommunityInvites(creatorId: number): Promise<(CommunityInvite & { company: Company })[]>;
  ...
  getBrandCreatorMembershipByCreatorAndCompany(creatorId: number, companyId: number): Promise<BrandCreatorMembership | undefined>;
  getCompanyMemberships(companyId: number, filters?: { status?: string; tierId?: number; search?: string }): Promise<(BrandCreatorMembership & { creator: User })[]>;
  getCreatorMemberships(creatorId: number): Promise<(BrandCreatorMembership & { company: Company })[]>;
  updateBrandCreatorMembership(id: number, data: Partial<BrandCreatorMembership>): Promise<BrandCreatorMembership>;
  getActiveMembershipCompanyIds(creatorId: number): Promise<number[]>;
  
  // Creator Discovery Profiles
  createOrUpdateDiscoveryProfile(data: InsertCreatorDiscoveryProfile): Promise<CreatorDiscoveryProfile>;
  getCompanyDiscoveryProfiles(companyId: number, filters?: { query?: string; connected?: 'all' | 'only' | 'none'; niche?: string; minFollowers?: number }): Promise<(CreatorDiscoveryProfile & { isConnected: boolean; isMember: boolean; creatorId?: number })[]>;
  ...
  getCompanyConversations(companyId: number, brandId: number, typeFilter?: ConversationType): Promise<any>;
  getConversationMessages(conversationId: number, limit?: number, offset?: number): Promise<any>;
  sendConversationMessage(conversationId: number, senderUserId: number, body: string): Promise<ConvMessage>;
  getUnreadConversationCount(userId: number, role: 'creator' | 'company', companyId?: number, brandId?: number): Promise<number>;
  getUnreadConversations(userId: number, role: 'creator' | 'company', companyId?: number): Promise<any[]>;
  getAllConversations(userId: number, role: 'creator' | 'company', companyId?: number): Promise<any[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByInstagram(handle: string): Promise<User | undefined> {
    ...
    const [user] = await db.select().from(users).where(
      sql`lower(replace(${users.instagram}, '@', '')) = ${normalizedHandle}`
    );
    return user;
  }
  ...
  async getCompanyCampaigns(companyId: number): Promise<Campaign[]> {
    return await db.select().from(campaigns).where(eq(campaigns.companyId, companyId)).orderBy(campaigns.createdAt);
  }
      ...
      if (campaign.targetAgeRanges && campaign.targetAgeRanges.length > 0) {
        ...
        if (hasWildcard || validRanges.length === 0) {
          ...
          const isInAgeRange = validRanges.some(({ min, max }) => {
            return creatorAge >= min && (max === undefined || max === Infinity ? true : creatorAge <= max);
          });
  ...
  async deleteCampaign(id: number): Promise<void> {
    await db.transaction(async (tx) => {
      const campaignApplications = await tx.select().from(applications).where(eq(applications.campaignId, id));
      const applicationIds = campaignApplications.map(app => app.id);
      
      if (applicationIds.length > 0) {
        await tx.delete(deliverableComments).where(inArray(deliverableComments.deliverableId, 
          tx.select({ id: deliverables.id }).from(deliverables).where(inArray(deliverables.applicationId, applicationIds))
        ));
  ...
  async getExistingApplication(campaignId: number, creatorId: number): Promise<Application | undefined> {
  ...
  async updateApplicationSeeding(id: number, seedingStatus: 'not_required' | 'pending' | 'sent' | 'received', trackingCode?: string, notes?: string): Promise<Application> {
  ...
  async updateApplicationCreatorWorkflowStatus(id: number, creatorWorkflowStatus: string): Promise<Application> {
    const [application] = await db.update(applications)
  ...
  // Campaign Briefing Operations
  async updateCampaignBriefing(id: number, briefingText: string | null, briefingMaterials: string[] | null): Promise<Campaign> {
  ...
  async addFavoriteCreator(companyId: number, creatorId: number): Promise<FavoriteCreator> {
    const [favorite] = await db.insert(favoriteCreators)
  ...
  async removeFavoriteCreator(companyId: number, creatorId: number): Promise<void> {
    await db.delete(favoriteCreators)
      .where(and(
        eq(favoriteCreators.companyId, companyId),
        eq(favoriteCreators.creatorId, creatorId)
      ));
  }
  ...
  async addFavoriteCompany(creatorId: number, companyId: number): Promise<FavoriteCompany> {
    const [favorite] = await db.insert(favoriteCompanies)
  ...
  async removeFavoriteCompany(creatorId: number, companyId: number): Promise<void> {
    await db.delete(favoriteCompanies)
      .where(and(
        eq(favoriteCompanies.creatorId, creatorId),
        eq(favoriteCompanies.companyId, companyId)
      ));
  }

  async getCreatorFavoriteCompanies(creatorId: number): Promise<(FavoriteCompany & { company: Company })[]> {
    const favorites = await db.select()
  ...
  async getCompanyRecentPartnerships(companyId: number, limit: number = 12): Promise<{
    id: number;
    campaignTitle: string;
    creatorName: string;
    creatorAvatar: string | null;
  ...
  // Discovery Operations
  async getDiscoverableBrands(options: { category?: string; featured?: boolean; limit?: number; offset?: number }): Promise<{
    id: number;
    name: string;
    tradeName: string | null;
    slug: string | null;
  ...
  async updateCampaignInvite(id: number, data: Partial<CampaignInvite>): Promise<CampaignInvite> {
    const [updated] = await db.update(campaignInvites)
      .set(data)
  ...
  async getExistingInvite(campaignId: number, creatorId: number): Promise<CampaignInvite | undefined> {
  ...
  async getCreatorPendingInvites(creatorId: number): Promise<(CampaignInvite & { campaign: Campaign; company: User })[]> {
  ...
  async getCreatorAllInvites(creatorId: number): Promise<(CampaignInvite & { campaign: Campaign; company: User })[]> {
    const results = await db.execute(sql`
      SELECT 
        ci.*,
        c.id as campaign_id,
  ...
  async updateInviteStatus(id: number, status: 'accepted' | 'declined'): Promise<CampaignInvite> {
    const [invite] = await db.update(campaignInvites)
  ...
  // Deliverable Comment Operations
  async createDeliverableComment(insertComment: InsertDeliverableComment): Promise<DeliverableComment> {
  ...
  async getDeliverableComments(deliverableId: number): Promise<(DeliverableComment & { user: User })[]> {
  ...
  async getUsersWithFilters(filters?: { role?: string; search?: string; isBanned?: boolean; sortBy?: 'name' | 'email' | 'role' | 'createdAt' | 'isBanned'; sortOrder?: 'asc' | 'desc' }): Promise<User[]> {
    const conditions = [];
  ...
  async updateProblemReport(id: number, updates: { status?: 'open' | 'in_progress' | 'resolved'; adminNotes?: string }): Promise<ProblemReport> {
  ...
  async getRecentActivity(limit: number = 10) {
    ...
    const recentApplicationsData = await db.select({
    ...
    const recentCampaignsData = await db.select({
  ...
  async getCompanyTemplates(companyId: number): Promise<CampaignTemplate[]> {
    return await db.select().from(campaignTemplates).where(eq(campaignTemplates.companyId, companyId)).orderBy(desc(campaignTemplates.updatedAt));
  }

  async updateCampaignTemplate(id: number, template: Partial<CampaignTemplate>): Promise<CampaignTemplate> {
    const [updatedTemplate] = await db.update(campaignTemplates)
  ...
  async updateCompanyMemberRole(companyId: number, userId: number, role: 'owner' | 'admin' | 'member'): Promise<CompanyMember> {
    const [updatedMember] = await db.update(companyMembers)
      .set({ role })
      .where(and(
        eq(companyMembers.companyId, companyId),
        eq(companyMembers.userId, userId)
  ...
  async getCompanyUserInvite(id: number): Promise<CompanyUserInvite | undefined> {
    const [invite] = await db.select().from(companyUserInvites).where(eq(companyUserInvites.id, id));
    return invite;
  }

  async getCompanyUserInviteByToken(token: string): Promise<(CompanyUserInvite & { company: Company }) | undefined> {
  ...
  async getCompanyPendingInvites(companyId: number): Promise<CompanyUserInvite[]> {
    return await db
      ...
      .from(companyUserInvites)
      .where(and(
        eq(companyUserInvites.companyId, companyId),
        eq(companyUserInvites.status, 'pending')
      ))
  ...
  async acceptCompanyUserInvite(token: string, userId: number): Promise<CompanyUserInvite> {
    const [updatedInvite] = await db.update(companyUserInvites)
      .set({ 
        status: 'accepted',
  ...
  async cancelCompanyUserInvite(id: number): Promise<void> {
    await db.update(companyUserInvites)
  ...
  async expireOldInvites(): Promise<void> {
    await db.update(companyUserInvites)
      ...
      .where(and(
        eq(companyUserInvites.status, 'pending'),
        sql`${companyUserInvites.expiresAt} < NOW()`
      ));
  }
  ...
  async createDefaultWorkflowStages(companyId: number): Promise<WorkflowStage[]> {
    ...
    const created = await Promise.all(
      defaultStages.map(stage =>
        this.createWorkflowStage({ ...stage, companyId })
      )
    );
    return created;
  }
  ...
  async getCreatorPosts(userId: number, platform?: 'instagram' | 'tiktok', limit: number = 12): Promise<CreatorPost[]> {
  ...
  async deleteOldCreatorPosts(userId: number, platform: 'instagram' | 'tiktok', keepCount: number): Promise<void> {
    ...
    if (keepIds.length > 0) {
      await db
        ...
        .where(and(
          eq(creatorPosts.userId, userId),
          eq(creatorPosts.platform, platform),
          sql`${creatorPosts.id} NOT IN (${sql.join(keepIds.map(id => sql`${id}`), sql`, `)})`
        ));
    }
  }

  async createAnalyticsHistoryEntry(entry: InsertCreatorAnalyticsHistory): Promise<CreatorAnalyticsHistory> {
    const [result] = await db
      .insert(creatorAnalyticsHistory)
      .values(entry)
      .returning();
    return result;
  }

  async getCreatorAnalyticsHistory(userId: number, platform?: 'instagram' | 'tiktok', limit: number = 30): Promise<CreatorAnalyticsHistory[]> {
  ...
  async getCreatorHashtags(userId: number, platform?: 'instagram' | 'tiktok', limit: number = 20): Promise<CreatorHashtag[]> {
  ...
  async updateCreatorHashtagStats(userId: number, platform: 'instagram' | 'tiktok', hashtags: { hashtag: string; avgEngagement: string }[]): Promise<void> {
  ...
  async updatePostAiInsight(postId: number, updates: Partial<InsertPostAiInsight>): Promise<PostAiInsight> {
    const [result] = await db
  ...
  async getCompanyPublicStats(companyId: number): Promise<{
    ...
    if (campaignIds.length > 0) {
      allApplications = await db
        .select()
        .from(applications)
        .where(sql`${applications.campaignId} IN (${sql.join(campaignIds.map(id => sql`${id}`), sql`, `)})`);
    }
    ...
    const completedCollaborations = allApplications.filter(
      a => a.status === 'accepted' && (a.workflowStatus === 'entregue' || a.workflowStatus === 'concluido')
    ).length;
    ...
    const respondedApps = allApplications.filter(a => 
      a.status !== 'pending' && a.appliedAt && a.updatedAt
    );
    ...
    const topCreators: { id: number; name: string; avatar: string | null; collaborations: number; avgRating: number }[] = [];
    ...
    if (campaignIds.length > 0) {
      allSales = await db
        .select()
        .from(salesTracking)
        .where(sql`${salesTracking.campaignId} IN (${sql.join(campaignIds.map(id => sql`${id}`), sql`, `)})`);
      
      allCommissions = await db
        .select()
        .from(creatorCommissions)
        .where(sql`${creatorCommissions.campaignId} IN (${sql.join(campaignIds.map(id => sql`${id}`), sql`, `)})`);
    }
    ...
    const pendingCommissions = allCommissions.filter(c => c.status === 'pending').reduce((sum, c) => sum + (c.amount || 0), 0);
    const paidCommissions = allCommissions.filter(c => c.status === 'paid').reduce((sum, c) => sum + (c.amount || 0), 0);
    ...
    const creatorSalesMap: Map<number, { sales: number; revenue: number; commissions: number }> = new Map();
    allSales.forEach(sale => {
      const current = creatorSalesMap.get(sale.creatorId) || { sales: 0, revenue: 0, commissions: 0 };
      current.sales += 1;
    ...
    const salesByCreator: { creatorId: number; name: string; avatar: string | null; sales: number; revenue: number; commissions: number }[] = [];
  ...
  async getFeatureFlagsByModule(module: string): Promise<FeatureFlag[]> {
    return await db.select().from(featureFlags).where(eq(featureFlags.module, module as any));
  }
  ...
  async initializeDefaultFeatureFlags(): Promise<void> {
    const defaultFlags: InsertFeatureFlag[] = [
      { name: 'gamification_enabled', description: 'Sistema de gamificação com pontos, níveis e badges', module: 'gamification', enabled: false },
      { name: 'leaderboard_enabled', description: 'Ranking mensal de criadores', module: 'gamification', enabled: false },
      { name: 'competitions_enabled', description: 'Competições entre criadores', module: 'gamification', enabled: false },
      { name: 'advanced_analytics_enabled', description: 'Análises avançadas e ROI de campanhas', module: 'advanced_analytics', enabled: false },
      { name: 'pdf_reports_enabled', description: 'Exportação de relatórios em PDF', module: 'advanced_analytics', enabled: false },
      { name: 'ecommerce_integration_enabled', description: 'Integração com Shopify/WooCommerce', module: 'ecommerce', enabled: false },
      { name: 'coupons_enabled', description: 'Sistema de cupons automáticos', module: 'ecommerce', enabled: false },
      { name: 'sales_tracking_enabled', description: 'Rastreamento de vendas e comissões', module: 'ecommerce', enabled: false },
      { name: 'social_listening_enabled', description: 'Monitoramento de menções à marca', module: 'social_listening', enabled: false },
      { name: 'hashtag_tracking_enabled', description: 'Rastreamento de hashtags', module: 'social_listening', enabled: false },
      { name: 'mention_alerts_enabled', description: 'Alertas de menções', module: 'social_listening', enabled: false },
    ];
  ...
  async getCreatorPointsHistory(creatorId: number, limit: number = 50): Promise<CreatorPointsEntry[]> {
    return await db.select().from(creatorPoints)
  ...
  async awardBadge(creatorId: number, badgeId: number): Promise<CreatorBadge> {
    const [created] = await db.insert(creatorBadges)
  ...
  async initializeDefaultLevelsAndBadges(): Promise<void> {
    ...
    if (existingLevels.length === 0) {
      const defaultLevels: InsertCreatorLevel[] = [
        { name: 'Bronze', minPoints: 0, maxPoints: 999, icon: '🥉', color: '#CD7F32', benefits: ['Acesso básico'] },
        { name: 'Prata', minPoints: 1000, maxPoints: 4999, icon: '🥈', color: '#C0C0C0', benefits: ['Destaque no perfil', 'Badge Prata'] },
        { name: 'Ouro', minPoints: 5000, maxPoints: 14999, icon: '🥇', color: '#FFD700', benefits: ['Prioridade em campanhas', 'Suporte VIP'] },
        { name: 'Diamante', minPoints: 15000, maxPoints: 49999, icon: '💎', color: '#B9F2FF', benefits: ['Campanhas exclusivas', '+10% comissões'] },
        { name: 'Lendário', minPoints: 50000, maxPoints: null, icon: '👑', color: '#9B59B6', benefits: ['Acesso total', '+20% comissões', 'Perfil verificado'] },
      ];
    ...
    if (existingBadges.length === 0) {
      const defaultBadges: InsertBadge[] = [
        { name: 'Primeira Campanha', description: 'Complete sua primeira campanha', icon: '🎉', requirement: 'first_campaign', requiredValue: 1 },
        { name: '10 Campanhas', description: 'Complete 10 campanhas', icon: '🏆', requirement: 'campaigns_completed', requiredValue: 10 },
        { name: 'Avaliação Perfeita', description: 'Receba nota 5 estrelas', icon: '⭐', requirement: 'perfect_rating', requiredValue: 1 },
        { name: 'Mestre da Pontualidade', description: '10 entregas no prazo seguidas', icon: '⏰', requirement: 'on_time_deliveries', requiredValue: 10 },
        { name: 'Rei do Engajamento', description: 'Alcance 10% de engajamento', icon: '👑', requirement: 'engagement_10_percent', requiredValue: 10 },
        { name: 'Post Viral', description: 'Alcance 100k views em um post', icon: '🚀', requirement: 'viral_100k', requiredValue: 100000 },
        { name: 'Campeão de Vendas', description: 'Gere 50 vendas', icon: '💰', requirement: 'sales_50', requiredValue: 50 },
        { name: 'Criador Verificado', description: 'Perfil verificado', icon: '✓', color: '#1DA1F2', requirement: 'verified_profile', requiredValue: 1 },
        { name: 'Top do Mês', description: 'Fique no top 3 do ranking mensal', icon: '🏅', requirement: 'top_3_monthly', requiredValue: 1 },
        { name: 'Sequência de 30 Dias', description: 'Mantenha atividade por 30 dias', icon: '🔥', requirement: 'streak_30', requiredValue: 30 },
      ];
  ...
  async getCampaignLeaderboardV1(campaignId: number): Promise<(CampaignCreatorStats & { creator: User })[]> {
  ...
  async getCampaignCreatorStats(campaignId: number, creatorId: number): Promise<CampaignCreatorStats | undefined> {
    const [stats] = await db.select()
      .from(campaignCreatorStats)
      .where(and(
  ...
  async upsertCampaignCreatorStats(stats: InsertCampaignCreatorStats): Promise<CampaignCreatorStats> {
    const existing = await this.getCampaignCreatorStats(stats.campaignId, stats.creatorId);
    
    if (existing) {
      const [updated] = await db.update(campaignCreatorStats)
        .set({ ...stats, updatedAt: new Date() })
        .where(eq(campaignCreatorStats.id, existing.id))
        .returning();
      return updated;
    } else {
      const [created] = await db.insert(campaignCreatorStats)
        .values(stats)
        .returning();
      return created;
    }
  ...
  async updateCampaignCreatorPoints(campaignId: number, creatorId: number, pointsDelta: number): Promise<CampaignCreatorStats> {
  ...
  async recalculateCampaignRankings(campaignId: number): Promise<void> {
    ...
    for (let i = 0; i < stats.length; i++) {
      await db.update(campaignCreatorStats)
        .set({ rank: i + 1 })
  ...
  async getCampaignCoupons(campaignId: number): Promise<(CampaignCoupon & { creator?: User })[]> {
    const coupons = await db.select().from(campaignCoupons).where(eq(campaignCoupons.campaignId, campaignId));
  ...
  async getBrandCoupons(brandId: number): Promise<(CampaignCoupon & { creatorName?: string; campaignTitle?: string })[]> {
  ...
  async getCreatorCoupon(campaignId: number, creatorId: number): Promise<CampaignCoupon | undefined> {
    const [coupon] = await db.select()
      .from(campaignCoupons)
      .where(and(
        eq(campaignCoupons.campaignId, campaignId),
        eq(campaignCoupons.creatorId, creatorId)
      ));
    return coupon;
  }

  async updateCoupon(id: number, updates: Partial<CampaignCoupon>): Promise<CampaignCoupon> {
    const [updated] = await db.update(campaignCoupons)
      .set(updates)
  ...
  async createSaleWithCommission(sale: { companyId: number; campaignId?: number | null; creatorId: number; couponId?: number | null; couponCode?: string; orderId: string; externalOrderId?: string; orderValue: number; commission?: number; commissionRateBps?: number; platform: 'shopify' | 'woocommerce' | 'manual'; rawJson?: Record<string, any> }): Promise<SalesTracking> {
    return await db.transaction(async (tx) => {
      const commissionAmount = sale.commission ?? (sale.commissionRateBps ? Math.round(sale.orderValue * sale.commissionRateBps / 10000) : 0);
      
      const [created] = await tx.insert(salesTracking).values({
        ...sale,
  ...
  async getBrandSales(companyId: number, fromDate?: Date, toDate?: Date): Promise<(SalesTracking & { creator: User })[]> {
    let query = db.select().from(salesTracking).where(eq(salesTracking.companyId, companyId));
    
    if (fromDate && toDate) {
      query = db.select().from(salesTracking).where(
        and(
  ...
  async getBrandCommissions(companyId: number): Promise<(CreatorCommission & { creator: User })[]> {
    const commissions = await db.select().from(creatorCommissions)
  ...
  async approveCommission(id: number): Promise<CreatorCommission> {
    const [updated] = await db.update(creatorCommissions)
  ...
  async getCampaignSales(campaignId: number): Promise<(SalesTracking & { creator: User })[]> {
    const sales = await db.select().from(salesTracking).where(eq(salesTracking.campaignId, campaignId)).orderBy(desc(salesTracking.trackedAt));
  ...
  async updateSaleStatus(id: number, status: 'pending' | 'confirmed' | 'paid' | 'cancelled'): Promise<SalesTracking> {
    const [updated] = await db.update(salesTracking)
      .set({ status })
  ...
  async getCommission(id: number): Promise<CreatorCommission | undefined> {
    const [commission] = await db.select().from(creatorCommissions).where(eq(creatorCommissions.id, id));
    return commission;
  }
  ...
  async getCampaignCommissions(campaignId: number): Promise<(CreatorCommission & { creator: User })[]> {
    const commissions = await db.select().from(creatorCommissions).where(eq(creatorCommissions.campaignId, campaignId)).orderBy(desc(creatorCommissions.createdAt));
  ...
  async updateCommissionStatus(id: number, status: 'pending' | 'approved' | 'paid'): Promise<CreatorCommission> {
  ...
  async getBrandSettingsByCompany(companyId: number): Promise<BrandSettings[]> {
    return db.select().from(brandSettings)
  ...
  async getOrCreateCompanyWallet(companyId: number): Promise<CompanyWallet> {
    ...
    if (!wallet) {
      ...
      const [created] = await db.insert(companyWallets).values({
        companyId,
        balance: 0,
        reservedBalance: 0,
  ...
  async updateCompanyWalletBalance(companyId: number, amount: number): Promise<CompanyWallet> {
    ...
    const [updated] = await db.update(companyWallets)
      .set({ 
  ...
  async addToCompanyWallet(companyId: number, amount: number, description: string): Promise<WalletTransaction> {
    const wallet = await this.getOrCreateCompanyWallet(companyId);
  ...
  async getWalletBoxes(companyWalletId: number): Promise<WalletBox[]> {
    return db.select().from(walletBoxes)
  ...
  async updateWalletBox(id: number, data: Partial<WalletBox>): Promise<WalletBox> {
    const [updated] = await db.update(walletBoxes)
      .set({ ...data, updatedAt: new Date() })
  ...
  async getCreatorBalance(userId: number): Promise<CreatorBalance | undefined> {
    const [balance] = await db.select().from(creatorBalances).where(eq(creatorBalances.userId, userId));
    return balance;
  }
  ...
  async getOrCreateCreatorBalance(userId: number): Promise<CreatorBalance> {
    ...
    if (!balance) {
      try {
        const [created] = await db.insert(creatorBalances).values({
          userId,
          availableBalance: 0,
          pendingBalance: 0,
        }).returning();
  ...
  async updateCreatorBalance(userId: number, data: Partial<CreatorBalance>): Promise<CreatorBalance> {
  ...
  async getCreatorTransactions(creatorBalanceId: number, limit?: number, offset?: number): Promise<WalletTransaction[]> {
    let query = db.select().from(walletTransactions)
  ...
  async getPaymentBatches(companyWalletId: number): Promise<PaymentBatch[]> {
    return db.select().from(paymentBatches)
  ...
  async payCreator(
    ...
    const [companyTransaction] = await db.insert(walletTransactions).values({
      companyWalletId: wallet.id,
      type: type as any,
      amount: -amount,
  ...
  async getBrandProgram(companyId: number): Promise<BrandProgram | undefined> {
    const [program] = await db.select().from(brandPrograms)
  ...
  async updateBrandProgram(companyId: number, updates: Partial<BrandProgram>): Promise<BrandProgram> {
    const [updated] = await db.update(brandPrograms)
      .set({ ...updates, updatedAt: new Date() })
      .where(eq(brandPrograms.companyId, companyId))
      .returning();
    return updated;
  ...
  async getBrandRewards(companyId: number): Promise<BrandReward[]> {
    return db.select().from(brandRewards)
  ...
  async getBrandReward(id: number): Promise<BrandReward | undefined> {
    const [reward] = await db.select().from(brandRewards)
  ...
  async updateBrandReward(id: number, updates: Partial<BrandReward>): Promise<BrandReward> {
    const [updated] = await db.update(brandRewards)
      .set({ ...updates, updatedAt: new Date() })
      .where(eq(brandRewards.id, id))
      .returning();
    return updated;
  ...
  async getBrandTiers(companyId: number): Promise<BrandTierConfig[]> {
    return db.select().from(brandTierConfigs)
  ...
  async getBrandTierConfig(id: number): Promise<BrandTierConfig | undefined> {
    const [tier] = await db.select().from(brandTierConfigs)
  ...
  async updateBrandTier(id: number, updates: Partial<BrandTierConfig>): Promise<BrandTierConfig> {
    const [updated] = await db.update(brandTierConfigs)
      .set({ ...updates, updatedAt: new Date() })
      .where(eq(brandTierConfigs.id, id))
      .returning();
    return updated;
  ...
  async getCampaignPrizes(campaignId: number): Promise<CampaignPrize[]> {
    return db.select().from(campaignPrizes)
  ...
  async getCampaignPrize(id: number): Promise<CampaignPrize | undefined> {
    const [prize] = await db.select().from(campaignPrizes)
  ...
  async updateCampaignPrize(id: number, updates: Partial<CampaignPrize>): Promise<CampaignPrize> {
    const [updated] = await db.update(campaignPrizes)
      .set(updates)
      .where(eq(campaignPrizes.id, id))
      .returning();
    return updated;
  ...
  async replaceCampaignPrizes(campaignId: number, prizes: InsertCampaignPrize[]): Promise<CampaignPrize[]> {
  ...
  async addPointsLedgerEntry(entry: InsertPointsLedgerEntry): Promise<PointsLedgerEntry | null> {
    ...
    const result = await db.insert(pointsLedger)
      .values(entry)
      .onConflictDoNothing({
        target: [pointsLedger.campaignId, pointsLedger.creatorId, pointsLedger.eventType, pointsLedger.refType, pointsLedger.refId]
      })
      .returning();
    
    if (result.length === 0) {
      // Entry already exists, skip updating score cache
      console.log(`[PointsLedger] Skipped duplicate entry: ${entry.eventType} for creator ${entry.creatorId}, ref ${entry.refType}:${entry.refId}`);
      return null;
    }
    return result[0];
  }
  ...
  async getPointsLedgerForCreator(creatorId: number, campaignId?: number): Promise<PointsLedgerEntry[]> {
  ...
  // Brand Creator Tiers
  async getBrandCreatorTier(companyId: number, creatorId: number): Promise<BrandCreatorTier | undefined> {
    const [result] = await db.select().from(brandCreatorTiers)
      .where(and(
        eq(brandCreatorTiers.companyId, companyId),
        eq(brandCreatorTiers.creatorId, creatorId)
      ));
    return result;
  }

  async updateBrandCreatorPoints(companyId: number, creatorId: number, deltaPoints: number): Promise<BrandCreatorTier> {
    ...
    if (existing) {
      ...
      const [updated] = await db.update(brandCreatorTiers)
        .set({ 
      ...
      const [created] = await db.insert(brandCreatorTiers)
  ...
  async getEffectiveScoringRules(campaignId: number, companyId: number): Promise<{
  ...
  async getEffectiveCaps(campaignId: number, companyId: number): Promise<{
  ...
  async getMetricSnapshot(campaignId: number, postId: string, platform: string): Promise<CampaignMetricSnapshot | undefined> {
    const [result] = await db.select().from(campaignMetricSnapshots)
      .where(and(
  ...
  async upsertMetricSnapshot(data: InsertCampaignMetricSnapshot): Promise<CampaignMetricSnapshot> {
    ...
    if (existing) {
      const [updated] = await db.update(campaignMetricSnapshots)
        .set({
          ...data,
      ...
      const [created] = await db.insert(campaignMetricSnapshots)
        .values(data)
        .returning();
      return created;
    }
  ...
  async getCampaignSnapshots(campaignId: number): Promise<CampaignMetricSnapshot[]> {
    return db.select().from(campaignMetricSnapshots)
      .where(eq(campaignMetricSnapshots.campaignId, campaignId))
      .orderBy(desc(campaignMetricSnapshots.lastSnapshotAt));
  }

  async getCreatorCampaignSnapshots(campaignId: number, creatorId: number): Promise<CampaignMetricSnapshot[]> {
    return db.select().from(campaignMetricSnapshots)
      .where(and(
        eq(campaignMetricSnapshots.campaignId, campaignId),
  ...
  async resetDailyPointsCounters(): Promise<void> {
    ...
    await db.update(campaignMetricSnapshots)
  ...
  async getCreatorPostsForCampaign(campaignId: number, allowedPlatforms: string[], windowDays: number): Promise<(CreatorPost & { creatorId: number })[]> {
    const windowStart = new Date();
    windowStart.setDate(windowStart.getDate() - windowDays);

    const acceptedApps = await db.select({ creatorId: applications.creatorId })
      .from(applications)
      .where(and(
        eq(applications.campaignId, campaignId),
        eq(applications.status, "accepted")
  ...
  async processMetricDelta(
    ...
    return await db.transaction(async (tx) => {
      ...
      const snapshotResult = await tx.select()
      ...
      let snapshot = snapshotResult[0] || null;
      ...
      const commentsDelta = Math.max(0, (post.comments || 0) - (snapshot?.lastAwardedComments || 0));
      ...
      if (snapshot) {
        await tx.update(campaignMetricSnapshots)
          .set({
            views: post.views || 0,
            likes: post.likes || 0,
  ...
  async getRewardEntitlements(filters: {
    ...
    return db.select().from(rewardEntitlements)
  ...
  async getRewardEntitlement(id: number): Promise<RewardEntitlement | undefined> {
    const [entitlement] = await db.select().from(rewardEntitlements)
  ...
  async approveRewardEntitlement(id: number, approvedBy: number): Promise<RewardEntitlement> {
    ...
    const [updated] = await db.update(rewardEntitlements)
      .set({ 
        status: "approved", 
        approvedBy, 
        approvedAt: new Date(),
  ...
  async rejectRewardEntitlement(id: number, rejectedBy: number, reason?: string): Promise<RewardEntitlement> {
    const [entitlement] = await db.select().from(rewardEntitlements)
      .where(eq(rewardEntitlements.id, id));
  ...
  async processCashReward(entitlementId: number, performedBy: number): Promise<void> {
    ...
    // Get company wallet
    const [wallet] = await db.select().from(companyWallets)
    ...
    const previousStatus = entitlement.status;
    await db.update(rewardEntitlements)
      .set({ 
  ...
  async getGamificationEnabledCampaigns(): Promise<{ id: number; companyId: number }[]> {
    const activeCampaigns = await db.select({ id: campaigns.id, companyId: campaigns.companyId })
      .from(campaigns)
      .where(eq(campaigns.status, 'open'));
    return activeCampaigns;
  }

  async getCreatorScore(campaignId: number, creatorId: number): Promise<{ totalPoints: number } | null> {
    const result = await db.select({
      totalPoints: sql<number>`COALESCE(SUM(${pointsLedger.deltaPoints}), 0)::int`,
    })
      .from(pointsLedger)
      .where(and(
        eq(pointsLedger.campaignId, campaignId),
        eq(pointsLedger.creatorId, creatorId)
      ));
  ...
  async checkMilestoneRewards(companyId: number, campaignId: number, creatorId: number, currentPoints: number): Promise<number> {
    // Get all milestone prizes for this campaign
    const prizes = await db.select().from(campaignPrizes)
      .where(and(
        eq(campaignPrizes.campaignId, campaignId),
        eq(campaignPrizes.type, "milestone")
      ))
    ...
    for (const prize of prizes) {
      ...
      if (currentPoints >= prize.milestonePoints) {
        ...
        const [existing] = await db.select().from(rewardEntitlements)
          .where(and(
  ...
  async getCreatorPendingCommunityInvites(creatorId: number): Promise<(CommunityInvite & { company: Company })[]> {
    const invites = await db.select().from(communityInvites)
      .where(and(
        eq(communityInvites.creatorId, creatorId),
        eq(communityInvites.status, "sent")
      ))
      .orderBy(desc(communityInvites.createdAt));
  ...
  async updateCommunityInvite(id: number, data: Partial<CommunityInvite>): Promise<CommunityInvite> {
    const [updated] = await db.update(communityInvites)
      .set(data)
      .where(eq(communityInvites.id, id))
      .returning();
    return updated;
  ...
  async createBrandCreatorMembership(data: InsertBrandCreatorMembership): Promise<BrandCreatorMembership> {
    const [membership] = await db.insert(brandCreatorMemberships).values({
      ...data,
      createdAt: new Date(),
      updatedAt: new Date(),
    }).returning();
  ...
  async getBrandCreatorMembership(id: number): Promise<BrandCreatorMembership | undefined> {
    const [membership] = await db.select().from(brandCreatorMemberships)
      .where(eq(brandCreatorMemberships.id, id));
    return membership;
  }

  async getBrandCreatorMembershipByCreatorAndCompany(creatorId: number, companyId: number): Promise<BrandCreatorMembership | undefined> {
    const [membership] = await db.select().from(brandCreatorMemberships)
      .where(and(
  ...
  async getCompanyMemberships(companyId: number, filters?: { status?: string; tierId?: number; search?: string }): Promise<(BrandCreatorMembership & { creator: User })[]> {
    ...
    for (const membership of memberships) {
      const [creator] = await db.select().from(users)
        .where(eq(users.id, membership.creatorId));
  ...
  async getCreatorMemberships(creatorId: number): Promise<(BrandCreatorMembership & { company: Company })[]> {
    const memberships = await db.select()
      .from(brandCreatorMemberships)
      .where(eq(brandCreatorMemberships.creatorId, creatorId))
      .orderBy(desc(brandCreatorMemberships.joinedAt));
  ...
  async updateBrandCreatorMembership(id: number, data: Partial<BrandCreatorMembership>): Promise<BrandCreatorMembership> {
    const [updated] = await db.update(brandCreatorMemberships)
      .set({ ...data, updatedAt: new Date() })
      .where(eq(brandCreatorMemberships.id, id))
      .returning();
    return updated;
  ...
  async getActiveMembershipCompanyIds(creatorId: number): Promise<number[]> {
    const memberships = await db.select({ companyId: brandCreatorMemberships.companyId })
      .from(brandCreatorMemberships)
      .where(and(
        eq(brandCreatorMemberships.creatorId, creatorId),
  ...
  async getCampaignPointsRules(campaignId: number): Promise<CampaignPointsRules | undefined> {
    const [rules] = await db.select().from(campaignPointsRules)
      .where(eq(campaignPointsRules.campaignId, campaignId));
    return rules;
  }

  async upsertCampaignPointsRules(campaignId: number, rulesJson: CampaignPointsRules['rulesJson'], overridesBrand: boolean = false): Promise<CampaignPointsRules> {
    const existing = await this.getCampaignPointsRules(campaignId);
    if (existing) {
      const [updated] = await db.update(campaignPointsRules)
        .set({ rulesJson, overridesBrand, updatedAt: new Date() })
        .where(eq(campaignPointsRules.campaignId, campaignId))
        .returning();
      return updated;
    } else {
      const [created] = await db.insert(campaignPointsRules)
  ...
  async getBrandLeaderboard(companyId: number, range: 'week' | 'month' | 'all', campaignId?: number, limit: number = 10): Promise<{
    creatorId: number;
    totalPoints: number;
    rank: number;
    creator?: User;
  ...
  async getCreatorPointsSummary(creatorId: number, companyId: number): Promise<{
    totalPoints: number;
    ...
    const [totalResult] = await db.select({
      ...
      .where(and(
        eq(pointsLedger.creatorId, creatorId),
        eq(pointsLedger.companyId, companyId)
      ));
    
    // Get points by event type
    const eventTypeResults = await db.select({
    ...
    // Get rank
    const allCreatorPoints = await db.select({
      creatorId: pointsLedger.creatorId,
      totalPoints: sql<number>`SUM(${pointsLedger.deltaPoints})::int`,
    })
      .from(pointsLedger)
    ...
    // Get recent entries
    const recentEntries = await db.select().from(pointsLedger)
      .where(and(
        eq(pointsLedger.creatorId, creatorId),
        eq(pointsLedger.companyId, companyId)
      ))
  ...
  async recordGamificationEvent(
    ...
    const entry = await db.insert(pointsLedger)
      .values({
        companyId,
        campaignId,
        creatorId,
  ...
  // Creator Discovery Profiles
  async createOrUpdateDiscoveryProfile(data: InsertCreatorDiscoveryProfile): Promise<CreatorDiscoveryProfile> {
    const existing = await db.select().from(creatorDiscoveryProfiles)
      .where(and(
    ...
    if (existing.length > 0) {
      const [updated] = await db.update(creatorDiscoveryProfiles)
  ...
  async getCompanyDiscoveryProfiles(
    companyId: number, 
    filters?: { query?: string; connected?: 'all' | 'only' | 'none'; niche?: string; minFollowers?: number }
  ): Promise<(CreatorDiscoveryProfile & { isConnected: boolean; isMember: boolean; creatorId?: number })[]> {
    const profiles = await db.select().from(creatorDiscoveryProfiles)
    ...
    const results = await Promise.all(profiles.map(async (profile) => {
      ...
      const linkedCreator = await db.select().from(users)
  ...
  async getCreatorCoursesWithProgress(creatorId: number): Promise<{
    ...
    return Promise.all(allCourses.map(async (course) => {
      const [progress] = await db.select().from(creatorCourseProgress)
        .where(and(
      ...
      if (moduleIds.length > 0) {
        ...
        if (lessonIds.length > 0) {
          const completedList = await db.select().from(creatorLessonProgress)
            .where(and(
  ...
  async startCourseForCreator(creatorId: number, courseId: number): Promise<CreatorCourseProgress> {
    ...
    const [existing] = await db.select().from(creatorCourseProgress)
      .where(and(
        eq(creatorCourseProgress.creatorId, creatorId),
    ...
    if (modules.length > 0) {
      const [firstLesson] = await db.select().from(courseLessons)
    ...
    const [progress] = await db.insert(creatorCourseProgress).values({
      creatorId,
      courseId,
      progressPct: 0,
  ...
  async completeLessonForCreator(creatorId: number, lessonId: number): Promise<{ lessonProgress: CreatorLessonProgress; courseProgress: CreatorCourseProgress }> {
    ...
    const [existingLessonProgress] = await db.select().from(creatorLessonProgress)
      .where(and(
        eq(creatorLessonProgress.creatorId, creatorId),
    ...
    if (existingLessonProgress) {
      ...
      [lessonProgress] = await db.insert(creatorLessonProgress).values({
        creatorId,
        lessonId,
      }).returning();
    }
    ...
    const completedLessons = await db.select().from(creatorLessonProgress)
      .where(and(
        eq(creatorLessonProgress.creatorId, creatorId),
        inArray(creatorLessonProgress.lessonId, allLessons.map(l => l.id))
      ));
    ...
    let [courseProgress] = await db.select().from(creatorCourseProgress)
      .where(and(
        eq(creatorCourseProgress.creatorId, creatorId),
        eq(creatorCourseProgress.courseId, courseId)
      ));
    ...
    if (!courseProgress) {
      [courseProgress] = await db.insert(creatorCourseProgress).values({
        creatorId,
        courseId,
        progressPct,
      ...
      [courseProgress] = await db.update(creatorCourseProgress)
        .set({
          progressPct,
          currentLessonId: nextLesson?.id || null,
          completedAt,
  ...
  async getCreatorLessonProgress(creatorId: number, courseId: number): Promise<number[]> {
    ...
    const completed = await db.select().from(creatorLessonProgress)
      .where(and(
        eq(creatorLessonProgress.creatorId, creatorId),
        inArray(creatorLessonProgress.lessonId, lessonIds)
      ));
  ...
  async listInspirations(filters: {
    ...
    if (conditions.length > 0) {
      return await db.select().from(inspirations).where(and(...conditions)).orderBy(desc(inspirations.createdAt));
    }
  ...
  async updateInspiration(id: number, data: Partial<InsertInspiration>): Promise<Inspiration | undefined> {
    const [updated] = await db.update(inspirations).set(data).where(eq(inspirations.id, id)).returning();
    return updated;
  }
  ...
  async saveInspiration(creatorId: number, inspirationId: number): Promise<CreatorSavedInspiration> {
    const existing = await db.select().from(creatorSavedInspirations)
      .where(and(
    ...
    const [saved] = await db.insert(creatorSavedInspirations).values({ creatorId, inspirationId }).returning();
    return saved;
  }

  async unsaveInspiration(creatorId: number, inspirationId: number): Promise<void> {
    await db.delete(creatorSavedInspirations).where(and(
      eq(creatorSavedInspirations.creatorId, creatorId),
      eq(creatorSavedInspirations.inspirationId, inspirationId)
    ));
  }

  async getCreatorSavedInspirations(creatorId: number): Promise<(CreatorSavedInspiration & { inspiration: Inspiration })[]> {
  ...
  // Collections
  async createInspirationCollection(creatorId: number, title: string): Promise<InspirationCollection> {
    const [collection] = await db.insert(inspirationCollections).values({ creatorId, title }).returning();
    return collection;
  }

  async getCreatorCollections(creatorId: number): Promise<(InspirationCollection & { itemCount: number })[]> {
    const collections = await db.select().from(inspirationCollections)
  ...
  async getCollectionWithItems(collectionId: number, creatorId: number): Promise<{
    collection: InspirationCollection;
    items: (InspirationCollectionItem & { inspiration: Inspiration })[];
  } | null> {
    const [collection] = await db.select().from(inspirationCollections)
      .where(and(
  ...
  async addToCollection(collectionId: number, inspirationId: number, creatorId: number): Promise<InspirationCollectionItem | null> {
    const [collection] = await db.select().from(inspirationCollections)
      .where(and(
        eq(inspirationCollections.id, collectionId),
        eq(inspirationCollections.creatorId, creatorId)
    ...
    const existing = await db.select().from(inspirationCollectionItems)
      .where(and(
    ...
    const [item] = await db.insert(inspirationCollectionItems)
      .values({ collectionId, inspirationId }).returning();
    return item;
  }

  async removeFromCollection(collectionId: number, inspirationId: number, creatorId: number): Promise<void> {
    const [collection] = await db.select().from(inspirationCollections)
      .where(and(
        eq(inspirationCollections.id, collectionId),
        eq(inspirationCollections.creatorId, creatorId)
    ...
    await db.delete(inspirationCollectionItems).where(and(
      eq(inspirationCollectionItems.collectionId, collectionId),
      eq(inspirationCollectionItems.inspirationId, inspirationId)
    ));
  }
  ...
  async deleteCollection(collectionId: number, creatorId: number): Promise<void> {
    await db.delete(inspirationCollections).where(and(
      eq(inspirationCollections.id, collectionId),
      eq(inspirationCollections.creatorId, creatorId)
    ));
  }

  async updateCollection(collectionId: number, creatorId: number, title: string): Promise<InspirationCollection | null> {
    const [updated] = await db.update(inspirationCollections)
      .set({ title })
      .where(and(
        eq(inspirationCollections.id, collectionId),
        eq(inspirationCollections.creatorId, creatorId)
  ...
  async addInspirationToCampaign(campaignId: number, inspirationId: number): Promise<CampaignInspiration> {
    const existing = await db.select().from(campaignInspirations)
      .where(and(
    ...
    const [item] = await db.insert(campaignInspirations)
      .values({ campaignId, inspirationId }).returning();
    return item;
  }

  async removeInspirationFromCampaign(campaignId: number, inspirationId: number): Promise<void> {
    await db.delete(campaignInspirations).where(and(
      eq(campaignInspirations.campaignId, campaignId),
      eq(campaignInspirations.inspirationId, inspirationId)
    ));
  }

  async getCampaignInspirations(campaignId: number): Promise<(CampaignInspiration & { inspiration: Inspiration })[]> {
    const items = await db.select({
  ...
  async getCreatorAddress(creatorId: number): Promise<CreatorAddress | undefined> {
    const [address] = await db.select().from(creatorAddresses)
  ...
  async updateCreatorAddress(id: number, creatorId: number, data: Partial<InsertCreatorAddress>): Promise<CreatorAddress | undefined> {
    const [address] = await db.update(creatorAddresses)
      .set({ ...data, updatedAt: new Date() })
  ...
  async getBrandReviewQueue(companyId: number): Promise<(Deliverable & { application: Application; campaign: Campaign; creator: User })[]> {
    const results = await db.select({
  ...
  async getBrandPendencias(companyId: number): Promise<{
    ...
    overdueDeliverables: (Deliverable & { application: Application; campaign: Campaign; creator: User })[];
  }> {
    const threeDaysAgo = new Date();
    threeDaysAgo.setDate(threeDaysAgo.getDate() - 3);

    // Get applications that are accepted but haven't progressed (still in 'aceito' status for 3+ days)
    const noResponseResults = await db.select({
      application: applications,
      campaign: campaigns,
      creator: users
    }).from(applications)
    ...
    const overdueResults: { deliverable: Deliverable; application: Application; campaign: Campaign; creator: User }[] = [];

    return {
      noResponse: noResponseResults.map(r => ({ ...r.application, campaign: r.campaign, creator: r.creator })),
  ...
  async getBrandPendingInvites(companyId: number): Promise<(CampaignInvite & { campaign: Campaign; creator: User })[]> {
    const results = await db.select({
  ...
  async getBrandDiscoveryQueue(companyId: number): Promise<(CreatorDiscoveryProfile & { creator: User })[]> {
  ...
  async updateConversationStatus(conversationId: number, status: 'open' | 'resolved'): Promise<void> {
    await db.update(conversations)
      .set({ status })
      .where(eq(conversations.id, conversationId));
  }

  async getCreatorConversations(creatorId: number, typeFilter?: ConversationType): Promise<{
    ...
    for (const conv of convList) {
      ...
      const [lastMsg] = await db.select().from(convMessages)
      ...
      const [readRecord] = await db.select().from(messageReads)
        .where(and(
          ...
          eq(messageReads.userId, creatorId)
        ));
      
      const unreadCount = await db.select({ count: sql<number>`count(*)` }).from(convMessages)
        .where(and(
      ...
      if (conv.type === 'campaign' && conv.campaignId) {
        const [campaign] = await db.select().from(campaigns).where(eq(campaigns.id, conv.campaignId));
  ...
  async getCompanyConversations(companyId: number, brandId: number, typeFilter?: ConversationType): Promise<{
    conversation: Conversation;
    ...
    for (const conv of convList) {
      ...
      const [companyUser] = await db.select().from(companyMembers)
        .where(eq(companyMembers.companyId, companyId));
      ...
      results.push({
        conversation: conv,
        creator: { id: creator.id, name: creator.name, avatar: creator.avatar, instagramHandle: creator.instagram },
        lastMessage: lastMsg ? { body: lastMsg.body, createdAt: lastMsg.createdAt } : null,
        unreadCount: Number(unreadCount[0]?.count || 0),
        campaignTitle,
      });
  ...
  async getConversationMessages(conversationId: number, limit = 50, offset = 0): Promise<(ConvMessage & { sender: { id: number; name: string; avatar: string | null } })[]> {
  ...
  async sendConversationMessage(conversationId: number, senderUserId: number, body: string): Promise<ConvMessage> {
    const [msg] = await db.insert(convMessages).values({
      conversationId,
      senderUserId,
      body,
    }).returning();
  ...
  async getUnreadConversationCount(userId: number, role: 'creator' | 'company', companyId?: number, brandId?: number): Promise<number> {
    let convList;
    if (role === 'creator') {
      convList = await db.select().from(conversations).where(eq(conversations.creatorId, userId));
    } else if (companyId && brandId) {
      convList = await db.select().from(conversations).where(and(
        eq(conversations.companyId, companyId),
        eq(conversations.brandId, brandId)
      ));
    } else if (companyId) {
      convList = await db.select().from(conversations).where(eq(conversations.companyId, companyId));
    } else {
      return 0;
    }
    ...
    for (const conv of convList) {
      ...
      const [unread] = await db.select({ count: sql<number>`count(*)` }).from(convMessages)
        .where(and(
          eq(convMessages.conversationId, conv.id),
          readRecord ? sql`${convMessages.createdAt} > ${readRecord.lastReadAt}` : sql`1=1`,
  ...
  async getUnreadConversations(userId: number, role: 'creator' | 'company', companyId?: number): Promise<any[]> {
    let whereClause;
    if (role === 'creator') {
    ...
    for (const conv of userConvs) {
      ...
      const [unreadResult] = await db.select({ count: sql<number>`count(*)` }).from(convMessages)
        .where(and(
          eq(convMessages.conversationId, conv.id),
          readRecord ? sql`${convMessages.createdAt} > ${readRecord.lastReadAt}` : sql`1=1`,
          ne(convMessages.senderUserId, userId)
      ...
      const [latestMsg] = await db.select().from(convMessages)
        .where(eq(convMessages.conversationId, conv.id))
        .orderBy(desc(convMessages.createdAt))
        .limit(1);

      const sender = latestMsg ? await db.select({ name: users.name }).from(users)
        .where(eq(users.id, latestMsg.senderUserId)).then(r => r[0]) : null;
  ...
  async getAllConversations(userId: number, role: 'creator' | 'company', companyId?: number): Promise<any[]> {
    let whereClause;
    if (role === 'creator') {
      whereClause = eq(conversations.creatorId, userId);
    } else if (companyId) {
  ...
  async getTags(type?: string): Promise<Tag[]> {
    if (type) {
      return await db.select().from(tags).where(eq(tags.type, type as any)).orderBy(tags.name);
    }
    return await db.select().from(tags).orderBy(tags.type, tags.name);
  }

  async getCreatorTags(creatorId: number): Promise<Tag[]> {
    const results = await db.select({ tag: tags })
      .from(creatorTags)
