...
// Helper function to download and save image to Object Storage
async function saveImageToStorage(imageUrl: string, platform: string, postId: string): Promise<string | null> {
  console.log(`[Storage] Attempting to save image for ${platform}/${postId}, URL: ${imageUrl?.substring(0, 100)}...`);
  
  if (!imageUrl) {
    console.log('[Storage] No imageUrl provided');
    return null;
  }
  
  try {
    const bucketId = process.env.DEFAULT_OBJECT_STORAGE_BUCKET_ID;
    console.log(`[Storage] Bucket ID: ${bucketId ? 'configured' : 'NOT configured'}`);
    
    if (!bucketId) {
      console.log('[Storage] No bucket configured, skipping image save');
      return imageUrl;
    }
    
    console.log(`[Storage] Fetching image from: ${imageUrl.substring(0, 80)}...`);
    ...
    if (!response.ok) {
      console.log(`[Storage] Failed to fetch image: ${response.status}`);
      return imageUrl;
    }
    ...
    console.log(`[Storage] Saving to: public/${fileName}`);
    ...
    const savedUrl = `/objects/uploads/public/${fileName}`;
    console.log(`[Storage] SUCCESS! Saved to: ${savedUrl}`);
    return savedUrl;
  } catch (error) {
    console.log(`[Storage] Error saving image: ${error}`);
    return imageUrl;
  }
}
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
  ...
  app.get("/api/creators/discovery-stats", async (req, res) => {
    ...
    try {
      ...
      const unvalidated = creatorsWithSocial.filter(c => {
        ...
        setImmediate(async () => {
          for (const creator of unvalidated) {
            try {
              ...
              console.error(`[Discovery Validation] Error validating ${creator.instagram}:`, err.message);
            }
          }
        });
      }
  ...
  app.get("/api/public/creator/:id", async (req, res) => {
    try {
      ...
      const completedApplicationsRaw = await db.select({
      ...
      .where(and(
        ...
        eq(applications.workflowStatus, 'entregue')
      ))
      ...
      const uniquePartners = completedApplications.reduce((acc, app) => {
  ...
  app.get("/api/discover/brands/:companyId", async (req, res) => {
    ...
    try {
      ...
      const openCampaigns = campaigns.filter(c => c.status === 'open' && c.visibility === 'public');
    ...
    try {
      ...
      const companiesData = await Promise.all(
        ids.map(async (id) => {
          ...
          if (company) {
            return { id: company.id, name: company.name, tradeName: company.tradeName, logo: company.logo };
          }
          return null;
        })
      );
  ...
  app.post("/api/campaigns/:campaignId/invites/bulk", async (req, res) => {
    ...
    res.json({
      ...
      summary: {
        ...
        skipped: results.alreadyInvited.length + results.alreadyApplied.length + results.invalidCreator.length,
  ...
  app.post("/api/invites/:id/accept", async (req, res) => {
    ...
    if (autoJoinCommunity) {
      const existingMembership = await storage.getBrandCreatorMembershipByCreatorAndCompany(creatorId, invite.companyId);
  ...
  app.post("/api/templates/:id/create-campaign", async (req, res) => {
    ...
    try {
      ...
      const campaignData = {
        ...
        structuredDeliverables: overrides.structuredDeliverables || template.structuredDeliverables || [],
  ...
  app.post("/api/applications", async (req, res) => {
    ...
    try {
      ...
      if (campaign) {
        ...
        const membersToNotify = companyMembers.filter(m => m.role === 'owner' || m.role === 'admin');
  ...
  app.patch("/api/applications/:id/status", async (req, res) => {
      ...
      if (status !== 'accepted' && status !== 'rejected') return res.status(400).send("Invalid status");
      if (!activeCompanyId) return res.status(400).json({ error: "Nenhuma loja ativa selecionada" });
          ...
          if (creatorUser?.email) {
            ...
            await sendApplicationApprovedEmail(
              ...
              creatorUser.name || 'Creator',
              {
                ...
                deadline: (campaign as any).endDate ? new Date((campaign as any).endDate).toLocaleDateString('pt-BR') : undefined,
  ...
  app.patch("/api/applications/:id/creator-workflow-status", async (req, res) => {
      ...
      const validStatuses = ["aceito", "contrato", "aguardando_produto", "producao", "revisao", "entregue"];
      ...
      if (campaign) {
        ...
        if (creatorWorkflowStatus === 'entregue') {
          ...
          if (previousStatus !== 'entregue') {
            ...
            try {
              const scoringRules = await storage.getEffectiveScoringRules(campaign.id, campaign.companyId);
              ...
              if (appDeliverables.length > 0 && Object.keys(scoringRules.pointsPerDeliverableType).length > 0) {
                for (const deliverable of appDeliverables) {
                  const dtype = (deliverable.deliverableType || 'other') as keyof typeof scoringRules.pointsPerDeliverableType;
                  const typePoints = scoringRules.pointsPerDeliverableType[dtype] ?? scoringRules.pointsPerDeliverable;
                  totalDeliverablePoints += typePoints;
                  typePointsDetails.push(`${dtype}: ${typePoints}pts`);
                }
              } else {
                totalDeliverablePoints = scoringRules.pointsPerDeliverable * Math.max(1, appDeliverables.length);
              }
              ...
              await storage.updateBrandCreatorPoints(campaign.companyId, req.user!.id, totalDeliverablePoints + (isOnTime ? scoringRules.pointsOnTimeBonus : 0));
  ...
  app.patch("/api/applications/:id/metrics", async (req, res) => {
    ...
    if (qualityScore !== undefined && (typeof qualityScore !== 'number' || qualityScore < 0 || qualityScore > 5)) {
    ...
    let stats = await storage.getCampaignCreatorStats(campaign.id, application.creatorId);
    ...
    if (stats) {
      ...
      const previousMetricPoints = previousViewPoints + previousEngagementPoints + previousSalesPoints + previousQualityPoints;
  ...
  app.post("/api/deliverables/upload", async (req, res) => {
    ...
    const handleUpload = () => new Promise<Express.Multer.File>((resolve, reject) => {
  ...
  app.get("/api/applications/:id/deliverables", async (req, res) => {
      ...
      const isCreator = req.user!.role === 'creator' && application.creatorId === Number(req.user!.id);
  ...
  app.post("/api/support/tickets", (req, res, next) => {
  ...
  app.get("/api/public/proxy-image", async (req, res) => {
      ...
      try {
          ...
          const isAllowedDomain = allowedDomains.some(domain => 
              url.hostname === domain || url.hostname.endsWith('.cdninstagram.com') || url.hostname.endsWith('.fbcdn.net')
          );
  ...
  app.post("/api/social/validate-instagram", async (req, res) => {
      ...
      try {
          const { downloadAndSaveToStorage, getPublicUrl } = await import("./services/instagram-profile-pic");
      ...
      if (user.instagramLastUpdated) {
          ...
          if (daysSinceUpdate < 30) {
              return res.status(429).json({ 
                  ...
                  nextUpdateDate: new Date(new Date(user.instagramLastUpdated).getTime() + 30 * 24 * 60 * 60 * 1000)
              });
          }
      }
  ...
  app.patch("/api/companies/:id/members/:userId", async (req, res) => {
    ...
    try {
      ...
      if (!isOwner) return res.status(403).json({ error: "Apenas o proprietário pode alterar funções" });
  ...
  app.post("/api/companies/:id/transfer-ownership", async (req, res) => {
    ...
    try {
      ...
      // Verify user is owner
      const isOwner = await storage.isCompanyOwner(companyId, req.user!.id);
      if (!isOwner) return res.status(403).json({ error: "Apenas o proprietário pode transferir a propriedade" });
  ...
  app.delete("/api/companies/:id/members/:userId", async (req, res) => {
    ...
    try {
      ...
      if (targetMember?.role === 'owner') {
        ...
        const isOwner = await storage.isCompanyOwner(companyId, req.user!.id);
        if (!isOwner) {
          return res.status(403).json({ error: "Apenas um proprietário pode remover outro proprietário" });
        }
        ...
        if (ownerCount <= 1) {
          return res.status(400).json({ error: "Não é possível remover o único proprietário. Promova outro membro primeiro." });
        }
      }
  ...
  app.post("/api/companies/:id/invites", async (req, res) => {
    ...
    try {
      ...
      // Verify user is admin/owner
      const isAdmin = await storage.isCompanyAdmin(companyId, req.user!.id);
      if (!isAdmin) return res.status(403).json({ error: "Apenas administradores podem convidar membros" });
  ...
  app.get("/api/admin/campaigns", isAdmin, async (req, res) => {
    try {
      ...
      const campaignsWithCounts = await Promise.all(filtered.map(async (campaign) => {
  ...
  app.get("/api/admin/campaigns/stats", isAdmin, async (req, res) => {
    try {
      ...
      campaignsList.forEach(c => {
        const budget = parseFloat(String(c.budget).replace(/[^\d.,]/g, '').replace(',', '.')) || 0;
        totalBudget += budget;
      });
  ...
  app.get("/api/admin/financial/stats", isAdmin, async (req, res) => {
    try {
      ...
      const pendingCommissionsCents = commissions.filter(c => c.status === 'pending').reduce((sum, c) => sum + (c.amount || 0), 0);
      const paidCommissionsCents = commissions.filter(c => c.status === 'paid').reduce((sum, c) => sum + (c.amount || 0), 0);
  ...
  app.get("/api/admin/financial/top-creators", isAdmin, async (req, res) => {
    try {
      ...
      const creatorMap = new Map<number, { totalSales: number; totalCommissions: number; ordersCount: number }>();
      
      sales.forEach(sale => {
        const current = creatorMap.get(sale.creatorId) || { totalSales: 0, totalCommissions: 0, ordersCount: 0 };
  ...
  app.get("/api/admin/campaigns/top-engagement", isAdmin, async (req, res) => {
    try {
      ...
      const campaignEngagement = await Promise.all(campaigns.map(async (campaign) => {
  ...
  app.get("/api/admin/campaigns/:id/rewards", isAdmin, async (req, res) => {
    try {
      ...
      const enrichedRewards = await Promise.all(rewards.map(async (reward) => {
        ...
        return {
          ...reward,
          creator: creator ? { id: creator.id, name: creator.name, avatar: creator.avatar, instagram: creator.instagram } : null,
  ...
  app.get("/api/admin/companies/top-engagement", isAdmin, async (req, res) => {
    try {
      ...
      const topCompanies = await Promise.all(companyIds.map(async (companyId) => {
        const company = await storage.getCompany(companyId);
  ...
  app.get("/api/creators/:creatorId/deep-analysis", async (req, res) => {
    try {
      ...
      const [instagramPosts, tiktokPosts, tiktokProfile, instagramHashtags, tiktokHashtags, analyticsHistory] = await Promise.all([
      ...
      if ((refreshPlatform === 'instagram' || refreshPlatform === 'both') && user.instagram) {
        try {
          ...
          for (const post of posts) {
            console.log(`[Deep Analysis] Processing post ${post.id}, thumbnailUrl exists: ${!!post.thumbnailUrl}`);
            const savedThumbnailUrl = await saveImageToStorage(post.thumbnailUrl, 'instagram', post.id);
      ...
      if ((refreshPlatform === 'tiktok' || refreshPlatform === 'both') && user.tiktok) {
        try {
          ...
          if (tiktokMetrics.exists) {
            ...
            // Store posts in database with saved thumbnails
            for (const post of posts) {
              const savedThumbnailUrl = await saveImageToStorage(post.thumbnailUrl, 'tiktok', post.id);
              await storage.upsertCreatorPost({
                userId: creatorId,
                platform: 'tiktok',
                postId: post.id,
  ...
  app.post("/api/posts/:postId/ai-insight", async (req, res) => {
    try {
      ...
      const postId = parseInt(req.params.postId);
      const { userId, platform, summary, strengths, improvements, hashtags, bestTimeToPost, audienceInsights, contentScore, engagementPrediction, recommendations } = req.body;
...
Seja específico, prático e focado em técnicas de criação de conteúdo. NUNCA mencione hashtags.`;
      ...
      let contentScore = Math.min(100, Math.round(
        (engagementRate * 15) + 
      ...
      if (comments > 50) {
        ...
        commentAnalysis = 'O alto volume de comentários indica que o conteúdo gerou forte reação da audiência.';
      } else if (comments >= 20) {
        commentSentiment = 'positivo';
        commentAnalysis = 'Bom nível de comentários. O público está interagindo de forma saudável.';
      } else if (comments >= 10) {
        commentSentiment = 'neutro';
        commentAnalysis = 'Nível moderado de comentários. Há espaço para incentivar mais interações.';
      } else {
      ...
      const insight = await storage.createPostAiInsight({
        ...
        audienceInsights,
        contentScore,
        engagementPrediction: engagementRate >= 3 ? 'Alto' : engagementRate >= 1 ? 'Médio' : 'Baixo',
        recommendations,
      });
  ...
  app.get("/api/feature-flags", async (req, res) => {
    try {
      const flags = await storage.getFeatureFlags();
      const publicFlags = flags.reduce((acc, flag) => {
  ...
  app.get("/api/gamification/status", async (req, res) => {
    try {
      ...
      res.json({ gamificationEnabled: false, leaderboardEnabled: false, competitionsEnabled: false });
    }
  });
  ...
  app.get("/api/gamification/my-progress", async (req, res) => {
    ...
    try {
      ...
      const totalPoints = 0;
      const currentLevel = levels.find(l => totalPoints >= l.minPoints && (!l.maxPoints || totalPoints < l.maxPoints)) || levels[0];
  ...
  app.post("/api/campaigns/:id/coupons/generate", async (req, res) => {
    ...
    for (const app of acceptedApps) {
      ...
      if (attempts >= maxAttempts) {
        console.error(`[Coupon] Failed to generate unique code for creator ${app.creatorId} after ${maxAttempts} attempts`);
        continue;
      }
    ...
    const connections = [
      {
        ...
        profileUrl: user.instagram ? `https://instagram.com/${user.instagram.replace('@', '')}` : undefined,
      },
      {
        platform: "tiktok",
  ...
  app.put("/api/brand/program", async (req, res) => {
    ...
    const { name, description, autoJoinCommunity, couponPrefix, couponGenerationRule, requirementsJson, gamificationEnabled, defaultRewardMode } = req.body;
  ...
  app.post("/api/brand/rewards", async (req, res) => {
    ...
    const { name, description, type, value, imageUrl, sku, stock, isActive, tierRequired, pointsCost } = req.body;
  ...
  app.post("/api/brand/tiers/reorder", async (req, res) => {
    ...
    try {
      ...
      await Promise.all(tierIds.map((id, index) => 
        storage.updateBrandTier(id, { sortOrder: index })
      ));
  ...
  app.get("/api/brand/leaderboard", async (req, res) => {
    ...
    const leaderboard = await storage.getBrandLeaderboard(activeCompanyId, range, campaignId, limit);
    res.json(leaderboard);
  });
  ...
  app.post("/api/gamification/event", async (req, res) => {
    ...
    const { companyId, creatorId, eventType, eventRefId, refType, refId, metadata, campaignId } = req.body;

    // Validate required fields
    if (!companyId || !creatorId || !eventType || !eventRefId || !refType || refId === undefined) {
      return res.status(400).json({ error: "Campos obrigatórios: companyId, creatorId, eventType, eventRefId, refType, refId" });
    }
  ...
  app.post("/api/system/points/emit-event", async (req, res) => {
    ...
    const eventTypeMap: Record<string, "post_created" | "reel_created" | "story_created" | "views_milestone" | "like_milestone" | "comment_milestone" | "sale_confirmed" | "delivery_approved" | "course_completed" | "admin_adjustment" | "ontime_bonus" | "quality_bonus" | "penalty_late" | "milestone_reached"> = {
      'post': 'post_created',
      'reels': 'reel_created',
      'story': 'story_created',
  ...
  app.post("/api/community/invites", async (req, res) => {
    ...
    if (creatorId) {
      const existingMembership = await storage.getBrandCreatorMembershipByCreatorAndCompany(creatorId, activeCompanyId);
    ...
    if (!resolvedCreatorId && creatorHandle) {
      ...
      if (creator) {
        ...
        // Check membership
        const existingMembership = await storage.getBrandCreatorMembershipByCreatorAndCompany(creator.id, activeCompanyId);
        if (existingMembership && existingMembership.status === "active") {
          return res.status(400).json({ error: "Creator já é membro ativo desta comunidade" });
        }
      }
  ...
  app.post("/api/join/:token/accept", async (req, res) => {
    ...
    if (req.user!.role !== 'creator') {
      return res.status(403).json({ error: "Apenas criadores podem aceitar convites de comunidade" });
    }
  ...
  app.get("/api/creator/communities", async (req, res) => {
    ...
    const enrichedMemberships = await Promise.all(activeMemberships.map(async (m) => {
      const tiers = await storage.getBrandTiers(m.companyId);
      const tier = m.tierId ? tiers.find(t => t.id === m.tierId) : null;
      return { 
        ...m, 
    ...
    try {
      ...
      for (const mem of memberships) {
        ...
        brandMap.set(company.id, {
          ...
          brandLogo: company.logo,
          membershipStatus: mem.status === "active" ? "member" : mem.status === "invited" ? "pending" : "none",
          membershipId: mem.id,
      ...
      for (const app of applications) {
        ...
        if (brandMap.has(companyId)) {
          ...
          if (app.status === "pending") {
            data.openCampaigns++;
          } else if (app.status === "accepted" && !["completed", "cancelled"].includes(app.workflowStatus || "")) {
      ...
      const brands = Array.from(brandMap.values())
        .filter(b => b.membershipStatus !== "none" || b.activeCampaigns > 0 || b.pendingInvites > 0)
  ...
  app.get("/api/creator/brand/:brandId/overview", async (req, res) => {
    ...
    try {
      ...
      const recentPointsEvents = membership ? 
        (await storage.getPointsLedgerForCreator(creatorId)).filter(e => e.companyId === brandId).slice(0, 10) : [];
      ...
      const sortedTiers = allTiers.sort((a, b) => a.minPoints - b.minPoints);
      const creatorTierIndex = creatorTierId ? sortedTiers.findIndex(t => t.id === creatorTierId) : -1;
      ...
      for (const app of brandApplications) {
        const campaign = (app as any).campaign || brandCampaigns.find(c => c.id === app.campaignId);
        if (!campaign) continue;
        ...
        } else if (app.status === "accepted" && !["completed", "cancelled", "entregue"].includes(app.workflowStatus || "")) {
  ...
  app.get("/api/creator/campaigns/active", async (req, res) => {
    ...
    try {
      ...
      const enrichedApps = await Promise.all(activeApps.map(async (app) => {
        const campaign = (app as any).campaign;
  ...
  app.get("/api/creator/overview", async (req, res) => {
    ...
    const activeCampaigns = applications
      ...
      .map(app => {
        ...
        else if (app.creatorWorkflowStatus === "aguardando_conteudo") nextAction = "Enviar conteúdo";
  ...
  app.post("/api/creator/collections/:collectionId/items", async (req, res) => {
    ...
    if (!item) return res.status(404).json({ error: "Coleção não encontrada ou não pertence a você" });
  ...
  app.get("/api/enrichment/cnpj/:cnpj", async (req, res) => {
    ...
    try {
      ...
      if (data.abertura) {
        ...
        const hoje = new Date();
        const diffAnos = Math.floor((hoje.getTime() - dataAbertura.getTime()) / (1000 * 60 * 60 * 24 * 365));
      ...
      else if (natureza.includes("EIRELI") || natureza.includes("Empresário Individual")) porte = "ME";
...
Responda APENAS com JSON válido (sem markdown, sem explicações):
{
  ...
  "logoUrl": "https://... ou null"
        ...
        const aiResponse = await sendGeminiMessage(prompt, {
          ...
          systemInstruction: "Você é um analista de dados que extrai informações estruturadas de websites. Sempre responda apenas com JSON válido, sem explicações."
        });
  ...
  app.get("/api/enrichment/completeness/:companyId", async (req, res) => {
    ...
    try {
      ...
      const suggestions = [];
      if (missingFields.includes("logo")) suggestions.push("Adicione um logo para melhorar o reconhecimento da marca");
      if (missingFields.includes("description")) suggestions.push("Adicione uma descrição para explicar o que sua empresa faz");
      if (missingFields.includes("instagram")) suggestions.push("Conecte seu Instagram para validar suas redes sociais");
      
      res.json({
        success: true,
        data: {
          score,
          level: score >= 80 ? "excellent" : score >= 60 ? "good" : score >= 40 ? "basic" : "incomplete",
          missingFields,
          suggestions,
        }
      });
  ...
  app.post("/api/enrichment/generate-description-v2", async (req, res) => {
    ...
    try {
      ...
      if (enrichmentContext?.websiteData) {
        ...
        if (web.description) contextParts.push(`Descrição do Site: ${web.description.substring(0, 300)}`);
...
Responda APENAS com JSON:
{
  ...
  app.get('/api/preview-email/:type', async (req, res) => {
    ...
    const mockCampaign = { 
      ...
      brandLogo: 'https://api.dicebear.com/7.x/initials/svg?seed=TP&backgroundColor=8b5cf6&textColor=ffffff',
      value: 'R$ 2.500', 
      deadline: '15/02/2026', 
      type: 'UGC' 
    };
