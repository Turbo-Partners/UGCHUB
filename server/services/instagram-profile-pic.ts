...
export async function getOrFetchProfilePic(username: string): Promise<ProfilePicResult> {
  ...
  try {
    const existing = await db.select().from(instagramProfiles).where(sql`LOWER(${instagramProfiles.username}) = ${cleanUsername}`).limit(1);
    ...
    if (localPicUrl) {
      ...
      console.log(`[ProfilePic] Local DB URL expired/failed for @${cleanUsername}, trying other sources...`);
    }
    ...
    console.log(`[ProfilePic] No pic found for @${cleanUsername} - use batch for Apify`);
...
export async function downloadAndSaveToStorage(username: string, imageUrl: string): Promise<string | null> {
  try {
    const { bucketId, publicPath } = getStorageConfig();
    
    const response = await fetch(imageUrl);
    if (!response.ok) {
      console.error(`[ProfilePic] Failed to download image: ${response.status}`);
      return null;
    }
    ...
    console.error(`[ProfilePic] Error saving to storage:`, error);
    return null;
  }
}

function parsePublicPath(publicPath: string, fileName: string): { bucketName: string; objectName: string } {
...
async function tryUserProfileAPIPic(targetUsername: string): Promise<string | null> {
  try {
    ...
    const contactResult = await db.select({
      ...
      .where(sql`LOWER(${instagramContacts.username}) = ${cleanUsername} AND ${instagramContacts.instagramUserId} IS NOT NULL`)
      .limit(1);

    let igsid: string | null = contactResult[0]?.igsid || null;
    let companyId: number | null = contactResult[0]?.companyId || null;
    ...
    if (!igsid) {
      const msgResult = await db.select({
        ...
        .where(sql`LOWER(${instagramMessages.senderUsername}) = ${cleanUsername} AND ${instagramMessages.senderId} IS NOT NULL AND ${instagramMessages.senderId} != ''`)
        .limit(1);
    ...
    if (companyId) {
      const [companyAccount] = await db.select({ accessToken: instagramAccounts.accessToken })
        .from(instagramAccounts)
        .where(sql`${instagramAccounts.companyId} = ${companyId} AND ${instagramAccounts.accessToken} IS NOT NULL`)
        .limit(1);
      account = companyAccount || null;
    }
    if (!account) {
      const [anyAccount] = await db.select({ accessToken: instagramAccounts.accessToken })
        .from(instagramAccounts)
    ...
    if (!response.ok) {
      console.log(`[ProfilePic UserProfileAPI] API error for @${cleanUsername} (IGSID: ${igsid}): ${response.status}`);
      return null;
    }
...
export async function batchGetOrFetchProfilePics(usernames: string[]): Promise<ProfilePicResult[]> {
  ...
  for (const username of uniqueUsernames) {
    try {
      const existing = await db.select().from(instagramProfiles)
      ...
      if (existing.length > 0 && existing[0].profilePicStoragePath) {
        ...
        if (storagePath) {
          await db.update(instagramProfiles)
            .set({ profilePicStoragePath: storagePath, profilePicOriginalUrl: record.profilePicUrl })
            .where(sql`LOWER(${instagramProfiles.username}) = ${username}`);
          results.set(username, {
            username,
            storagePath,
  ...
  console.log(`[ProfilePic Batch] ${results.size} found in cache, ${needLocalDB.length} need local DB check`);
  ...
  if (needUserProfileAPI.length > 0) {
    console.log(`[ProfilePic Batch] ${needUserProfileAPI.length} need User Profile API (IGSID) check`);
  ...
  if (needApify.length > 0) {
    ...
    try {
      ...
      console.log(`[ProfilePic Batch] Apify returned ${apifyResults.length} results for ${needApify.length} usernames`);
    } catch (apifyError) {
  ...
  console.log(`[ProfilePic Batch] Complete: ${results.size} total, ${needApify.length > 0 ? '1 Apify call' : '0 Apify calls'}`);
