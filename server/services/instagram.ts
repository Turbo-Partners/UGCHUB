...
async function getProfilePicFromLocalData(username: string): Promise<string | null> {
  ...
  if (creator) {
    if (creator.instagramProfilePic?.startsWith('/api/storage/')) return creator.instagramProfilePic;
    if (creator.avatar?.startsWith('/api/storage/')) return creator.avatar;
  }
  
  const [cached] = await db.select({ storagePath: instagramProfiles.profilePicStoragePath })
...
export function isMessageIncoming(params: {
  ...
  const { senderUsername, senderId, accountUsername, accountInstagramUserId, accountFacebookUserId } = params;
...
export class InstagramService {
  ...
  async exchangeCodeForToken(code: string, type: "creator" | "business"): Promise<InstagramTokenResponse> {
    const redirectUri = this.getRedirectUri(type);
  ...
  async sendDirectMessage(accessToken: string, recipientId: string, message: string): Promise<any> {
    const response = await fetch(
      `${INSTAGRAM_GRAPH_BASE_URL}/me/messages`,
      {
        method: "POST",
        headers: { 
  ...
  async getConversations(accessToken: string, userId: string, onPageFetched?: (page: number, conversations: any[], hasMore: boolean) => void): Promise<any[]> {
    ...
    while (nextUrl && pageCount < maxPages) {
      ...
      console.log(`[Instagram] Page ${pageCount}: Got ${conversations.length} conversations, total: ${allConversations.length}, hasMore: ${hasMore}`);
    ...
    console.log(`[Instagram] Total fetched: ${allConversations.length} conversations from ${pageCount} pages`);
    
    return allConversations;
  }

  // Fetch messages from Instagram API for a specific conversation
  async fetchConversationMessagesFromAPI(accessToken: string, conversationId: string, limit: number = 100): Promise<any[]> {
    try {
      const response = await fetch(
      ...
      const data = await response.json();
      console.log(`[Instagram] Fetched ${data.data?.length || 0} messages from conversation ${conversationId.substring(0, 20)}...`);
      return data.data || [];
    } catch (e) {
  ...
  async getInstagramBusinessAccountFromPage(accessToken: string, pageId: string): Promise<string | null> {
    const response = await fetch(
  ...
  async saveInstagramAccount(data: InsertInstagramAccount): Promise<number> {
    const [result] = await db.insert(instagramAccounts).values(data).returning({ id: instagramAccounts.id });
    return result.id;
  }

  async getInstagramAccountByUserId(userId: number): Promise<typeof instagramAccounts.$inferSelect | null> {
    const [account] = await db.select().from(instagramAccounts).where(eq(instagramAccounts.userId, userId)).limit(1);
    return account || null;
  }

  async getInstagramAccountByCompanyId(companyId: number): Promise<typeof instagramAccounts.$inferSelect | null> {
    const [account] = await db.select().from(instagramAccounts).where(eq(instagramAccounts.companyId, companyId)).limit(1);
    return account || null;
  }

  async getInstagramAccountByInstagramUserId(instagramUserId: string): Promise<typeof instagramAccounts.$inferSelect | null> {
  ...
  async updateInstagramAccount(id: number, data: Partial<InsertInstagramAccount>): Promise<void> {
    await db.update(instagramAccounts).set({ ...data, updatedAt: new Date() }).where(eq(instagramAccounts.id, id));
  }
  ...
  async saveMessage(data: InsertInstagramMessage): Promise<number> {
    const [result] = await db.insert(instagramMessages).values(data).returning({ id: instagramMessages.id });
    return result.id;
  }

  async savePost(data: InsertInstagramPost): Promise<number> {
    const [result] = await db.insert(instagramPosts).values(data).returning({ id: instagramPosts.id });
    return result.id;
  }

  // DM Inbox Methods
  async getMessagesForAccount(instagramAccountId: number): Promise<(typeof instagramMessages.$inferSelect)[]> {
    return db.select()
  ...
  async getConversationsList(instagramAccountId: number, limit: number = 200, offset: number = 0): Promise<{
    ...
    const [account] = await db.select({ username: instagramAccounts.username })
    ...
    const lastMessages = await db.execute<{
      ...
      FROM instagram_messages
      WHERE instagram_account_id = ${instagramAccountId} 
        AND conversation_id IN (${sql.join(conversationIds.map(id => sql`${id}`), sql`, `)})
    ...
    if (usernameList.length > 0) {
      const profilePics = await db.select({
        ...
        .from(instagramProfiles)
        .where(sql`LOWER(${instagramProfiles.username}) IN (${sql.join(usernameList.map(u => sql`${u}`), sql`, `)})`);
    ...
    return summaries.rows.map(summary => {
      ...
      if (participant) {
        ...
        const senderIsAccount = accountUsername && lastMsg.sender_username?.toLowerCase() === accountUsername;
      ...
      const isIncoming = lastMsg?.is_incoming ?? 
        (lastMsg?.sender_username?.toLowerCase() !== accountUsername);
  ...
  async getConversationMessages(instagramAccountId: number, conversationId: string): Promise<(typeof instagramMessages.$inferSelect)[]> {
    return db.select()
      .from(instagramMessages)
      .where(
        and(
  ...
  async markMessagesAsRead(instagramAccountId: number, conversationId: string): Promise<void> {
    await db.update(instagramMessages)
      .set({ isRead: true })
      .where(
        and(
          eq(instagramMessages.instagramAccountId, instagramAccountId),
  ...
  async markAllMessagesAsRead(instagramAccountId: number): Promise<number> {
    const result = await db.update(instagramMessages)
      .set({ isRead: true })
      .where(
        and(
          eq(instagramMessages.instagramAccountId, instagramAccountId),
  ...
  async getUnreadConversationCount(instagramAccountId: number): Promise<number> {
    ...
    const result = await db.select({ count: sql<number>`count(distinct ${instagramMessages.conversationId})::int` })
      .from(instagramMessages)
      .where(
        and(
          eq(instagramMessages.instagramAccountId, instagramAccountId),
  ...
  async syncConversationsFromAPI(
    ...
    try {
      let totalFetched = 0;
      const conversations = await this.getConversations(accessToken, instagramUserId, (page, pageConvs, hasMore) => {
      ...
      const allMsgIds = conversations.flatMap(c => (c.messages?.data || []).map((m: any) => m.id));
      const existingMsgIds = new Set<string>();
      if (allMsgIds.length > 0) {
        const chunkSize = 500;
        for (let i = 0; i < allMsgIds.length; i += chunkSize) {
          const chunk = allMsgIds.slice(i, i + chunkSize);
          const rows = await db.select({ messageId: instagramMessages.messageId })
            .from(instagramMessages)
            .where(sql`${instagramMessages.messageId} IN (${sql.join(chunk.map(id => sql`${id}`), sql`, `)})`);
          for (const row of rows) {
            existingMsgIds.add(row.messageId);
          }
        }
      }
      console.log(`[Instagram DM Sync] Pre-loaded ${existingMsgIds.size} existing message IDs from ${allMsgIds.length} total`);
        ...
        try {
          ...
          if (otherParticipant?.id && otherParticipantUsername) {
            ...
            if (this.profilePicCache.has(cacheKey)) {
              ...
              otherParticipantProfilePic = await getProfilePicFromLocalData(otherParticipantUsername);
              
              if (!otherParticipantProfilePic) {
                try {
                  otherParticipantProfilePic = await this.getUserProfilePic(accessToken, otherParticipant.id);
                  if (otherParticipantProfilePic) {
                    console.log(`[Instagram DM Sync] Got pic via User Profile API for @${otherParticipantUsername}`);
                  }
                } catch {}
              }
          ...
          for (const msg of newMessages) {
            try {
              ...
              let messageType: "text" | "image" | "video" | "audio" | "file" | "share" | "story_mention" | "story_reply" = "text";
  ...
  async syncPostsAndComments(instagramAccountId: number, companyId?: number): Promise<{ postsSync: number; commentsSync: number; errors: number }> {
    let postsSync = 0;
    let commentsSync = 0;
    let errors = 0;
                      ...
                      for (const reply of comment.replies.data) {
                        if (reply.username) {
                          ...
                          await recordInteraction({
                            ...
                            metadata: { postId: post.id, permalink: post.permalink, parentCommentId: comment.id },
                          });
                        }
                      }
                    }
    ...
    console.log(`[Instagram] Synced ${postsSync} posts and ${commentsSync} comments with ${errors} errors`);
    return { postsSync, commentsSync, errors };
  }

  async getEngagementByUsername(instagramAccountId: number, username: string): Promise<{ commentsCount: number; repliesCount: number }> {
  ...
  async saveProfileToCache(data: {
    ...
    await db.insert(instagramProfiles)
      .values({
  ...
  async getMessageStats(instagramAccountId: number, username: string): Promise<{ 
  ...
  async syncFullHistory(
    ...
    try {
      ...
      while (hasMore && totalPages < maxPages) {
        ...
        for (const post of posts) {
          try {
            ...
            if (existingPost.length === 0) {
              await db.insert(instagramPosts)
                .values({
                  instagramAccountId,
                  instagramMediaId: post.id,
                  mediaType: post.media_type,
              ...
              await db.update(instagramPosts)
                .set({
                  ...
                  mentionedAccounts: mentionedAccounts.length > 0 ? mentionedAccounts : existingPost[0].mentionedAccounts,
        ...
        if (onProgress) {
          onProgress({ page: totalPages, totalFetched: postsSync, done: !hasMore || totalPages >= maxPages });
        }

        console.log(`[Instagram FullSync] Page ${totalPages}: ${posts.length} posts fetched, ${postsSync} saved/updated`);
      ...
      console.log(`[Instagram FullSync] Complete: ${postsSync} posts, ${totalPages} pages, ${errors} errors`);
    } catch (err) {
