...
async function main() {
  ...
  const userResult = await db.execute(
    sql`SELECT id, name, role FROM core.users WHERE email = ${EMAIL}`,
  );
  ...
  const compResult = await db.execute(
    sql`SELECT company_id FROM company.company_members WHERE user_id = ${uid}`,
  );
  ...
  if (campaignIds.length > 0) {
    ...
    console.log('\n--- Campanhas ---');
    await del(
      `DELETE FROM campaign.deliverable_comments WHERE deliverable_id IN (SELECT id FROM campaign.deliverables WHERE application_id IN (SELECT id FROM campaign.applications WHERE campaign_id IN (${cids})))`,
      'deliverable_comments',
    );
    await del(
      `DELETE FROM campaign.deliverables WHERE application_id IN (SELECT id FROM campaign.applications WHERE campaign_id IN (${cids}))`,
      'deliverables',
    );
    await del(`DELETE FROM campaign.applications WHERE campaign_id IN (${cids})`, 'applications');
    await del(
      `DELETE FROM campaign.campaign_invites WHERE campaign_id IN (${cids})`,
      'campaign_invites',
    );
    await del(
      `DELETE FROM campaign.campaign_hashtags WHERE campaign_id IN (${cids})`,
      'campaign_hashtags',
    );
    await del(
      `DELETE FROM campaign.campaign_coupons WHERE campaign_id IN (${cids})`,
      'campaign_coupons',
    );
    await del(
      `DELETE FROM content.campaign_inspirations WHERE campaign_id IN (${cids})`,
      'campaign_inspirations',
    );
    await del(
      `DELETE FROM campaign.campaign_prizes WHERE campaign_id IN (${cids})`,
      'campaign_prizes',
    );
    await del(
      `DELETE FROM campaign.campaign_creator_stats WHERE campaign_id IN (${cids})`,
      'campaign_creator_stats',
    );
    await del(
      `DELETE FROM analytics.campaign_metric_snapshots WHERE campaign_id IN (${cids})`,
      'campaign_metric_snapshots',
    );
    await del(
      `DELETE FROM campaign.campaign_points_rules WHERE campaign_id IN (${cids})`,
      'campaign_points_rules',
    );
  ...
  for (const cid of companyIds) {
    ...
    // campaign.* schema
    await del(
      `DELETE FROM campaign.campaign_templates WHERE company_id = ${cid}`,
      'campaign_templates',
    );
    ...
    await del(
      `DELETE FROM misc.favorite_companies WHERE company_id = ${cid}`,
      'favorite_companies',
    );
    ...
    await del(
      `DELETE FROM misc.post_ai_insights WHERE user_id IN (SELECT user_id FROM company.company_members WHERE company_id = ${cid})`,
      'post_ai_insights',
    );
    ...
    await del(
      `DELETE FROM billing.creator_commissions WHERE company_id = ${cid}`,
      'creator_commissions',
    );
    await del(
      `DELETE FROM billing.wallet_transactions WHERE company_id = ${cid}`,
      'wallet_transactions',
    );
    ...
    await del(
      `DELETE FROM gamification.reward_entitlements WHERE company_id = ${cid}`,
      'reward_entitlements',
    );
    await del(
      `DELETE FROM gamification.brand_programs WHERE company_id = ${cid}`,
      'brand_programs',
    );
    await del(`DELETE FROM gamification.brand_rewards WHERE company_id = ${cid}`, 'brand_rewards');
    await del(
      `DELETE FROM gamification.brand_tier_configs WHERE company_id = ${cid}`,
      'brand_tier_configs',
    );

    // brand.* schema
    await del(
      `DELETE FROM brand.brand_creator_tiers WHERE company_id = ${cid}`,
      'brand_creator_tiers',
    );
    await del(
      `DELETE FROM brand.brand_creator_memberships WHERE company_id = ${cid}`,
    ...
    // content.* schema
    await del(
      `DELETE FROM content.creator_saved_inspirations WHERE inspiration_id IN (SELECT id FROM content.inspirations WHERE brand_id = ${cid})`,
      'creator_saved_inspirations (brand)',
    );
    await del(
      `DELETE FROM content.campaign_inspirations WHERE campaign_id IN (SELECT id FROM campaign.campaigns WHERE company_id = ${cid})`,
      'campaign_inspirations',
    );
    ...
    // messaging.* schema
    await del(
      `DELETE FROM messaging.message_reads WHERE conversation_id IN (SELECT id FROM messaging.conversations WHERE company_id = ${cid})`,
      'message_reads',
    );
    await del(
      `DELETE FROM messaging.conv_messages WHERE conversation_id IN (SELECT id FROM messaging.conversations WHERE company_id = ${cid})`,
      'conv_messages',
    );
    ...
    await del(
      `DELETE FROM creator.creator_discovery_profiles WHERE company_id = ${cid}`,
      'creator_discovery_profiles',
    );
    await del(
      `DELETE FROM creator.creator_auth_links WHERE company_id = ${cid}`,
      'creator_auth_links',
    );
    await del(
      `DELETE FROM creator.creator_ad_partners WHERE company_id = ${cid}`,
      'creator_ad_partners',
    );
    ...
    await del(
      `DELETE FROM social.instagram_interactions WHERE company_id = ${cid}`,
      'instagram_interactions',
    );
    await del(
      `DELETE FROM social.instagram_contacts WHERE company_id = ${cid}`,
      'instagram_contacts',
    );
    await del(
      `DELETE FROM social.instagram_messages WHERE company_id = ${cid}`,
      'instagram_messages',
    );
    await del(
      `DELETE FROM social.instagram_tagged_posts WHERE company_id = ${cid}`,
      'instagram_tagged_posts',
    );
    await del(`DELETE FROM social.instagram_posts WHERE company_id = ${cid}`, 'instagram_posts');
    await del(
      `DELETE FROM social.instagram_profiles WHERE company_id = ${cid}`,
      'instagram_profiles',
    );
    await del(
      `DELETE FROM social.instagram_accounts WHERE company_id = ${cid}`,
      'instagram_accounts',
    );
    await del(
      `DELETE FROM social.meta_ad_accounts_list WHERE company_id = ${cid}`,
      'meta_ad_accounts_list',
    );
    await del(`DELETE FROM social.meta_ad_accounts WHERE company_id = ${cid}`, 'meta_ad_accounts');
    await del(
      `DELETE FROM social.meta_business_managers WHERE company_id = ${cid}`,
    ...
    await del(
      `DELETE FROM system.data_source_registry WHERE company_id = ${cid}`,
      'data_source_registry',
    );
    await del(
      `DELETE FROM system.notifications WHERE user_id IN (SELECT user_id FROM company.company_members WHERE company_id = ${cid})`,
      'notifications (company users)',
    );

    // company.* schema
    await del(
      `DELETE FROM company.company_user_invites WHERE company_id = ${cid}`,
  ...
  await del(
    `DELETE FROM analytics.creator_analytics_history WHERE user_id = ${uid}`,
    'creator_analytics_history',
  );
  ...
  await del(
    `DELETE FROM content.creator_saved_inspirations WHERE creator_id = ${uid}`,
    'creator_saved_inspirations',
  );
  await del(
    `DELETE FROM academy.creator_course_progress WHERE creator_id = ${uid}`,
    'creator_course_progress',
  );
  await del(
    `DELETE FROM academy.creator_lesson_progress WHERE creator_id = ${uid}`,
  ...
  // Sessions
  await del(
    `DELETE FROM system.session WHERE sess::text LIKE '%"passport":{"user":${uid}}%'`,
    'sessions',
  );
